package calendar;

import java.time.Duration;
import java.util.List;
import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.chrome.ChromeDriver;
import org.openqa.selenium.support.ui.ExpectedConditions;
import org.openqa.selenium.support.ui.WebDriverWait;

public class SelectDateOnCalendar {

	public static void main(String[] args) {

		selectDate("July", 2023, 16);
	}

	public static void selectDate(String mon, int yr, int day){
		
		WebDriver driver;
		WebDriverWait wait;

		//initialize the date values
		String inputMonth = mon;
		String intpuYear = String.valueOf(yr);
		String inputDay = String.valueOf(day);


		//initialize chrome browser and wait
		driver = new ChromeDriver();
		wait = new WebDriverWait(driver, Duration.ofSeconds(20));
		driver.manage().window().maximize();
		
		//Go to url
		driver.get("https://www.goibibo.com/");
		driver.manage().timeouts().pageLoadTimeout(Duration.ofSeconds(10));

		//click on the calendar
		driver.findElement(By.xpath("//span[@class='sc-12foipm-21 lerOcM fswDownArrow']")).click();

		//get the month year value
		WebElement element = driver.findElement(By.xpath("//div[@class='DayPicker-Month'][1]//div[@class='DayPicker-Caption']"));
		String monthYear = element.getText();
		String[] monthYEAR = monthYear.split(" ");
		String month = monthYEAR[0];
		String year = monthYEAR[1];
		System.out.println(month+" "+year);

		//click next button if needed then update month year values
		while(!month.equalsIgnoreCase(inputMonth) || !year.equals(intpuYear)) {
			driver.findElement(By.xpath("//span[@aria-label='Next Month']")).click();
			monthYear = driver.findElement(By.xpath("//div[@class='DayPicker-Month'][1]//div[@class='DayPicker-Caption']")).getText();

			//set an explicit wait
			wait.until(ExpectedConditions.elementToBeClickable(element));

			monthYEAR = monthYear.split(" ");
			month = monthYEAR[0];
			year = monthYEAR[1];
		}
		//get the list of all date-elements
		List <WebElement> dateElements = driver.findElements(By.xpath("//div[@class='DayPicker-Day']"));
		for(WebElement dateElement : dateElements) {
			String date = dateElement.getText();
			if(date.equalsIgnoreCase(inputDay)) {
				dateElement.click();
				System.out.println("departure date : "+date+" is selected");
				break;
			}
		}	 
	}

}
